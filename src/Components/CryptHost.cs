...
internal class CryptHost(Game game) : GameComponent(game)
{
  ...
  private void TryInitialize()
  {
    ...
    else
    {
      ...
      int[] second = new int[4]{ 0, 1, 2, 3 };
      foreach (int key in keys.Except<int>((IEnumerable<int>) second).ToArray<int>())
      ...
      foreach (Volume volume in (IEnumerable<Volume>) this.LevelManager.Volumes.Values)
      {
        if (volume.Id > 1 && (volume.Id != 14 || this.TraversedVolumes.Count != 3))
        {
          ...
          int num = RandomHelper.InList<int>(this.LevelManager.Volumes.Keys.Except<int>((IEnumerable<int>) new int[3]
          {
            0,
            1,
            volume.Id
          ...
          scriptTrigger.Event = "Enter";
          scriptTrigger.Object = new Entity()
          {
            Type = "Volume",
