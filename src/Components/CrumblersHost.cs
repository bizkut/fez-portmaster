...
public class CrumblersHost : GameComponent
{
  ...
  private void TryInitialize()
  {
    ...
    foreach (TrileInstance instance in this.LevelManager.Triles.Values.Where<TrileInstance>((Func<TrileInstance, bool>) (x => x.Trile.ActorSettings.Type == ActorType.Crumbler)))
    {
  ...
  {
    if (this.GameState.Loading || this.GameState.Paused || this.GameState.InMap || !this.CameraManager.ActionRunning || !this.CameraManager.Viewpoint.IsOrthographic() || !this.AnyCrumblers)
      return;
  ...
  private void TestForCrumblers()
  {
    ...
    if (this.PlayerManager.Grounded)
    {
      ...
      if (nl != null && (flag |= nl.Trile.ActorSettings.Type == ActorType.Crumbler) && !this.States.Any<CrumblersHost.CrumblerState>((Func<CrumblersHost.CrumblerState, bool>) (x => x.Instance == nl)))
        instance = nl;
      else if (fh != null && (flag |= fh.Trile.ActorSettings.Type == ActorType.Crumbler) && !this.States.Any<CrumblersHost.CrumblerState>((Func<CrumblersHost.CrumblerState, bool>) (x => x.Instance == fh)))
        instance = fh;
    }
    else if (this.PlayerManager.Action.IsOnLedge())
    {
      TrileInstance hi = this.PlayerManager.HeldInstance;
      if (hi != null && (flag |= hi.Trile.ActorSettings.Type == ActorType.Crumbler) && !this.States.Any<CrumblersHost.CrumblerState>((Func<CrumblersHost.CrumblerState, bool>) (x => x.Instance == hi)))
        instance = hi;
    }
    if (instance != null)
    ...
    if (!flag || SettingsManager.Settings.DisableController)
      return;
    this.Gamepads[this.GameState.ActivePlayer].Vibrate(VibrationMotor.LeftLow, 0.25, TimeSpan.FromSeconds(0.10000000149011612));
    this.Gamepads[this.GameState.ActivePlayer].Vibrate(VibrationMotor.RightHigh, 0.25, TimeSpan.FromSeconds(0.10000000149011612));
  }
  ...
  private class CrumblerState
  {
    ...
    public CrumblerState(TrileInstance instance, CrumblersHost host)
    {
      ...
      host.sWarning.EmitAt(this.OriginalCenter, RandomHelper.Centered(0.0099999997764825821));
    }
    ...
    public void Rumble()
    {
      ...
      {
        this.c = 0;
        Vector3 vector3 = new Vector3(RandomHelper.Centered(0.039999999105930328), 0.0f, RandomHelper.Centered(0.039999999105930328));
    ...
    {
      this.Host.sCrumble.EmitAt(this.OriginalCenter, RandomHelper.Centered(0.0099999997764825821));
      ...
      if (this.LevelManager.ScreenSpaceLimits.TryGetValue(key, out limit))
      {
        ...
        while ((flag2 ? id.Z : id.X) != limit.End)
        {
          ...
          if (toRemove != null && !toRemove.Hidden && toRemove.Trile.ActorSettings.Type == ActorType.Crumbler)
          {
            ...
            this.ParticleSystems.Add(this.System = new TrixelParticleSystem(ServiceHelper.Game, new TrixelParticleSystem.Settings()
            {
    ...
    private void ClearTriles()
    {
      ...
      {
        if (this.PlayerManager.Action.IsOnLedge() && this.PlayerManager.HeldInstance == instance)
        {
