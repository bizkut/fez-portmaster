...
public class BigWaterfallHost : GameComponent, IBigWaterfallService, IScriptingBase
{
  ...
  private void TryInitialize()
  {
    ...
    this.WaterfallPlane = this.LevelManager.BackgroundPlanes.Values.FirstOrDefault<BackgroundPlane>((Func<BackgroundPlane, bool>) (x => x.ActorType == ActorType.BigWaterfall));
    this.Enabled = this.WaterfallPlane != null;
    if (!this.Enabled)
      return;
    this.MoriaPlane = this.LevelManager.BackgroundPlanes.Values.FirstOrDefault<BackgroundPlane>((Func<BackgroundPlane, bool>) (x => x.TextureName == "MORIA_GLOW"));
    ...
    this.MoriaPlane.Position -= Vector3.UnitX * (1f / 1000f);
    ...
    this.LevelMaterializer.StaticPlanesMesh.GroupOrder = (Comparison<Group>) ((x, y) =>
    {
      if (x == this.MoriaPlane.Group)
        return 1;
      return y != this.MoriaPlane.Group ? oldGo(x, y) : -1;
    });
    ...
    this.Top = (this.WaterfallPlane.Position + this.WaterfallPlane.Scale * this.WaterfallPlane.Size / 2f).Dot(Vector3.UnitY);
    ...
    {
      this.SwapOpened();
      this.eWaterfallOpen = this.CMProvider.CurrentLevel.Load<SoundEffect>("Sounds/MiscActors/BigWaterfallOpen").EmitAt(this.WaterfallPlane.Position, true, 0.0f, 0.0f);
    }
    else
      this.eWaterfallClosed = this.CMProvider.CurrentLevel.Load<SoundEffect>("Sounds/MiscActors/BigWaterfallClosed").EmitAt(this.WaterfallPlane.Position, true, 0.0f, 0.0f);
  }
  ...
  public LongRunningAction Open(int id)
  {
    ...
    this.GameState.Save();
    SoundEmitter eWaterfallOpen = this.CMProvider.CurrentLevel.Load<SoundEffect>("Sounds/MiscActors/BigWaterfallOpen").EmitAt(this.WaterfallPlane.Position, true, true);
    ...
    this.sWaterfallOpening.EmitAt(this.WaterfallPlane.Position);
