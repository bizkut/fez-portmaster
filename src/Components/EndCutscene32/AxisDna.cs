...
internal class AxisDna : DrawableGameComponent
{
  ...
  private void Reset()
  {
    ...
    for (int index = 0; index < 750; ++index)
    {
      ...
      float num = (float) ((1.0 - Math.Pow(((double) index - 375.0) / 375.0, 2.0)) * 0.20000000298023224);
      Group group = this.HelixMesh.AddColoredBox(new Vector3(num, 70f, num) / 10000f, Vector3.Zero, EndCutscene32Host.PurpleBlack, true);
    ...
    {
      vertices[index * 2] = new FezVertexPositionColor(new Vector3((float) random.NextDouble() - 0.5f, (float) random.NextDouble() - 0.5f, 0.0f), new Color((int) (byte) random.Next(0, 256 /*0x0100*/), (int) (byte) random.Next(0, 256 /*0x0100*/), (int) (byte) random.Next(0, 256 /*0x0100*/), 0));
    ...
    BufferedIndexedPrimitives<FezVertexPositionColor> indexedPrimitives1 = new BufferedIndexedPrimitives<FezVertexPositionColor>(vertices, indices, PrimitiveType.LineList);
    ...
    {
      this.FatAxisMesh.Effect = (BaseEffect) new DefaultEffect.VertexColored();
      this.HelixMesh.Effect = (BaseEffect) new DefaultEffect.VertexColored();
      this.NoiseMesh.Effect = (BaseEffect) (this.ShimmeringEffect = new ShimmeringPointsEffect());
  ...
  public override void Update(GameTime gameTime)
  {
    ...
    switch (this.ActiveState)
    {
      case AxisDna.State.AxisZoom:
        ...
        float linearStep = FezMath.Saturate(this.Time / 10f);
        this.CameraManager.Direction = FezMath.Slerp(Vector3.UnitZ, -Vector3.Transform(Vector3.UnitZ, this.HelixMesh.Rotation), linearStep * 0.5f);
        this.FatAxisMesh.Material.Opacity = 1f - Easing.EaseIn((double) FezMath.Saturate(linearStep * 2f), EasingType.Quadratic);
        if ((double) this.Time != 0.0)
          this.FatAxisMesh.Scale *= MathHelper.Lerp(1.015f, 1.01f, Easing.EaseIn((double) linearStep, EasingType.Quadratic));
        this.HelixMesh.Scale = this.FatAxisMesh.Scale;
        this.CameraManager.Center = new Vector3(0.0f, this.FatAxisMesh.Scale.X / 1000f, 0.0f);
        this.CameraManager.SnapInterpolation();
        this.HelixMesh.Rotation *= Quaternion.CreateFromAxisAngle(Vector3.UnitZ, Easing.EaseOut(0.014999999664723873 * (1.0 - (double) linearStep), EasingType.Cubic));
        this.NoiseMesh.Material.Opacity = Easing.EaseIn((double) linearStep, EasingType.Cubic);
  ...
  private void ChangeState()
  {
    ...
    {
      foreach (DrawableGameComponent scene in this.Host.Scenes)
      {
        if (scene is TetraordialOoze)
          (scene as TetraordialOoze).NoiseMesh = this.NoiseMesh;
      }
      this.Host.Cycle();
    }
    else
