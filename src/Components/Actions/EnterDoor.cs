...
public class EnterDoor(Game game) : PlayerAction(game)
{
  ...
  protected override void LoadContent()
  {
    ...
    {
      Mesh fadeOutQuad = this.fadeOutQuad;
      fadeOutQuad.Effect = (BaseEffect) new DefaultEffect.VertexColored()
      {
  ...
  protected override void TestConditions()
  {
    switch (this.PlayerManager.Action)
    {
      ...
      case ActionType.IdleYawn:
        ...
        if (this.InputManager.RotateLeft == FezButtonState.Down || this.InputManager.RotateRight == FezButtonState.Down)
        {
          this.UnDotize();
          break;
        }
        if (this.PlayerManager.NextLevel == "SKULL_B" && ServiceHelper.Get<ITombstoneService>().get_AlignedCount() < 4)
        {
          this.UnDotize();
          break;
        }
        if (this.PlayerManager.NextLevel == "ZU_HEADS" && !this.GameState.SaveData.World.ContainsKey("ZU_HEADS"))
        {
          ISuckBlockService suckBlockService = ServiceHelper.Get<ISuckBlockService>();
          bool flag = false;
          for (int id = 2; id < 6; ++id)
          {
            if (!suckBlockService.get_IsSucked(id))
              flag = true;
          }
          if (flag)
          {
        ...
        if (this.PlayerManager.DoorVolume.HasValue && this.PlayerManager.Grounded && !this.PlayerManager.HideFez && this.PlayerManager.CanControl && !this.PlayerManager.Background && !this.DotManager.PreventPoI && this.GameState.SaveData.World.ContainsKey(key) && !this.skipPreview && key != this.LevelManager.Name && this.LevelManager.Name != "CRYPT" && this.LevelManager.Name != "PYRAMID")
        {
          if (MemoryContentManager.AssetExists("Other Textures\\map_screens\\" + key.Replace('/', '\\')))
          {
            ...
            if (key == "SEWER_QR" || key == "ZU_HOUSE_QR")
              this.DotManager.DestinationVignetteSony = this.CMProvider.CurrentLevel.Load<Texture2D>($"Other Textures/map_screens/{key}_SONY");
        ...
        if (spinThroughDoor)
        {
          ...
          if ((double) this.PlayerManager.Position.Dot(b) < (double) a.Dot(b))
  ...
  private void UnDotize()
  {
    ...
    this.DotManager.DestinationVignetteSony = (Texture2D) null;
    this.DotManager.Burrow();
  }

  private Vector3 GetDestination()
  {
    if (this.PlayerManager.DoorVolume.HasValue)
    {
  ...
  protected override void Begin()
  {
    ...
    else
    {
      ...
      if (this.PlayerManager.SpinThroughDoor)
      {
        this.CameraManager.ChangeViewpoint(this.CameraManager.Viewpoint.GetRotatedView(1), 2f);
      ...
      if (this.LevelManager.DestinationIsFarAway)
      {
        ...
        Vector3 vector3 = this.CameraManager.Viewpoint.RightVector() * vector2.X + Vector3.Up * vector2.Y;
        Vector3 destinationCenter = new Vector3(this.PlayerManager.Position.X, this.PlayerManager.Position.Y + num, this.PlayerManager.Position.Z) + vector3 * 2f;
  ...
  {
    return type.IsEnteringDoor() || (double) this.step != -1.0;
  }
  ...
  protected override bool Act(TimeSpan elapsed)
  {
    ...
    if (this.PlayerManager.SpinThroughDoor && this.hasFlipped && !this.hasChangedLevel)
    {
      this.step = 0.0f;
    }
    else
    {
      ...
      float num = this.PlayerManager.SpinThroughDoor ? 0.75f : 1.25f;
      if (this.hasChangedLevel)
        num *= 0.75f;
      this.step = (float) this.transitionTime.TotalSeconds / num;
    }
    ...
    if (this.PlayerManager.SpinThroughDoor && this.hasFlipped && this.CameraManager.ActionRunning && !this.hasChangedLevel)
    {
      ...
      this.CameraManager.ChangeViewpoint(this.CameraManager.Viewpoint.GetRotatedView(-1), 0.0f);
      this.CameraManager.SnapInterpolation();
      this.CameraManager.ChangeViewpoint(this.CameraManager.Viewpoint.GetRotatedView(1));
  ...
  private void DoLoad(bool dummy)
  {
    ...
    {
      if (this.PlayerManager.CarriedInstance != null)
        this.PlayerManager.Action = this.PlayerManager.CarriedInstance.Trile.ActorSettings.Type.IsHeavy() ? ActionType.ExitDoorCarryHeavy : ActionType.ExitDoorCarry;
      else
        this.PlayerManager.Action = ActionType.ExitDoor;
    }
    if (!this.skipFade)
  ...
  public override void Draw(GameTime gameTime)
  {
    ...
    if (this.PlayerManager.CarriedInstance != null && !this.PlayerManager.SpinThroughDoor)
    {
      ...
      this.GraphicsDevice.PrepareStencilRead(CompareFunction.Equal, FezEngine.Structure.StencilMask.NoSilhouette);
