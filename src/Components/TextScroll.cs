...
public class TextScroll : DrawableGameComponent
{
  ...
  private void LateLoadContent()
  {
    ...
    Vector2 vector2 = this.GTR.MeasureWithGlyphs(big, this.Text, scale);
    this.MiddlePartSize = vector2 * new Vector2(1f, 1.25f);
    int x = (int) this.MiddlePartSize.X;
    int y = (int) this.MiddlePartSize.Y;
    Vector2 middlePartSize = this.MiddlePartSize;
    if (this.TextTexture != null)
    {
      this.TextTexture.Dispose();
      this.TextTexture = (RenderTarget2D) null;
    }
    this.TextTexture = new RenderTarget2D(this.GraphicsDevice, x, y, false, this.GraphicsDevice.PresentationParameters.BackBufferFormat, this.GraphicsDevice.PresentationParameters.DepthStencilFormat, 0, RenderTargetUsage.PreserveContents);
    using (SpriteBatch spriteBatch = new SpriteBatch(this.GraphicsDevice))
    {
      ...
      this.GTR.DrawString(spriteBatch, big, this.Text, middlePartSize / 2f - vector2 / 2f + new Vector2(0.0f, this.FontManager.TopSpacing / 2f), TextScroll.TextColor, scale);
      spriteBatch.End();
  ...
  public override void Update(GameTime gameTime)
  {
    ...
    if (!this.Ready)
      this.LateLoadContent();
    this.ScrollMesh.Scale = new Vector3(96f / (float) this.GraphicsDevice.Viewport.Width, 96f / (float) this.GraphicsDevice.Viewport.Height, 1f) * this.GraphicsDevice.GetViewScale();
    this.TextGroup.Scale = Vector3.One;
    if (Culture.IsCJK)
      this.TextGroup.Scale /= 2f;
    this.ScrollMesh.Position = new Vector3(0.01f, this.OnTop ? 0.6125f : -0.6125f, 0.0f);
    if (this.Timeout.HasValue)
    {
      float? nullable = this.Timeout;
      float totalSeconds = (float) gameTime.ElapsedGameTime.TotalSeconds;
      this.Timeout = nullable.HasValue ? new float?(nullable.GetValueOrDefault() - totalSeconds) : new float?();
