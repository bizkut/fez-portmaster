...
public class GlyphTextRenderer
{
  ...
  public GlyphTextRenderer(Game game)
  {
    ...
    this.ButtonImages.Add("BACK_SONY", new GlyphTextRenderer.GlyphDescription(global.Load<Texture2D>(str1 + "BackButton_SONY"), md5));
    this.ButtonImages.Add("START_SONY", new GlyphTextRenderer.GlyphDescription(global.Load<Texture2D>(str1 + "StartButton_SONY"), md5));
    this.ButtonImages.Add("A_SONY", new GlyphTextRenderer.GlyphDescription(global.Load<Texture2D>(str1 + "AButton_SONY"), md1));
    this.ButtonImages.Add("B_SONY", new GlyphTextRenderer.GlyphDescription(global.Load<Texture2D>(str1 + "BButton_SONY"), md1));
    this.ButtonImages.Add("X_SONY", new GlyphTextRenderer.GlyphDescription(global.Load<Texture2D>(str1 + "XButton_SONY"), md1));
    this.ButtonImages.Add("Y_SONY", new GlyphTextRenderer.GlyphDescription(global.Load<Texture2D>(str1 + "YButton_SONY"), md1));
    this.ButtonImages.Add("LS_SONY", new GlyphTextRenderer.GlyphDescription(global.Load<Texture2D>(str1 + "LeftStick_SONY"), md3));
    this.ButtonImages.Add("RS_SONY", new GlyphTextRenderer.GlyphDescription(global.Load<Texture2D>(str1 + "RightStick_SONY"), md3));
    this.ButtonImages.Add("UP_SONY", new GlyphTextRenderer.GlyphDescription(global.Load<Texture2D>(str1 + "DPadUp_SONY"), md3));
    this.ButtonImages.Add("DOWN_SONY", new GlyphTextRenderer.GlyphDescription(global.Load<Texture2D>(str1 + "DPadDown_SONY"), md3));
    this.ButtonImages.Add("LEFT_SONY", new GlyphTextRenderer.GlyphDescription(global.Load<Texture2D>(str1 + "DPadLeft_SONY"), md3));
    this.ButtonImages.Add("RIGHT_SONY", new GlyphTextRenderer.GlyphDescription(global.Load<Texture2D>(str1 + "DPadRight_SONY"), md3));
    this.ButtonImages.Add("RB_PS3", new GlyphTextRenderer.GlyphDescription(global.Load<Texture2D>(str1 + "RightBumper_PS3"), md6));
    this.ButtonImages.Add("LB_PS3", new GlyphTextRenderer.GlyphDescription(global.Load<Texture2D>(str1 + "LeftBumper_PS3"), md6));
    this.ButtonImages.Add("RT_PS3", new GlyphTextRenderer.GlyphDescription(global.Load<Texture2D>(str1 + "RightTrigger_PS3"), md2));
    this.ButtonImages.Add("LT_PS3", new GlyphTextRenderer.GlyphDescription(global.Load<Texture2D>(str1 + "LeftTrigger_PS3"), md2));
    this.ButtonImages.Add("RB_PS4", new GlyphTextRenderer.GlyphDescription(global.Load<Texture2D>(str1 + "RightBumper_PS4"), md6));
    this.ButtonImages.Add("LB_PS4", new GlyphTextRenderer.GlyphDescription(global.Load<Texture2D>(str1 + "LeftBumper_PS4"), md6));
    this.ButtonImages.Add("RT_PS4", new GlyphTextRenderer.GlyphDescription(global.Load<Texture2D>(str1 + "RightTrigger_PS4"), md2));
    this.ButtonImages.Add("LT_PS4", new GlyphTextRenderer.GlyphDescription(global.Load<Texture2D>(str1 + "LeftTrigger_PS4"), md2));
    ...
    float scale)
  {
    ...
    float y1 = font.MeasureString("X").Y;
    foreach (GlyphTextRenderer.GlyphLocation glyphLocation in this.GlyphLocations)
    {
      ...
      {
        Texture2D image = glyphDescription.Image;
        ...
        vector2_1.Y = y2 - vector2_1.Y * 0.5f;
        Vector2 vector2_2 = position + vector2_1;
        float a = y1 / (float) image.Height * scale;
        if (image.Height <= 32 /*0x20*/)
        {
          double num1 = (double) a;
          a = (float) Math.Ceiling((double) a);
          double num2 = (double) a;
          if (num1 == num2 && (double) this.game.GraphicsDevice.GetViewScale() >= 2.0)
            ++a;
        }
        if (Culture.IsCJK && Culture.Language != Language.Chinese)
          a *= 1.2f;
        if (Culture.IsCJK)
          vector2_2.Y -= (float) ((double) image.Height * (double) a * 0.05000000074505806);
        Rectangle destinationRectangle = new Rectangle(FezMath.Round((double) vector2_2.X - (double) image.Width * (double) a / 2.0), FezMath.Round((double) vector2_2.Y - (double) image.Height * (double) a / 2.0), FezMath.Round((double) image.Width * (double) a), FezMath.Round((double) image.Height * (double) a));
        batch.Draw(image, destinationRectangle, new Color((int) byte.MaxValue, (int) byte.MaxValue, (int) byte.MaxValue, (int) color.A));
      }
    }
    if (!Culture.IsCJK)
      return;
  ...
  public Vector2 MeasureWithGlyphs(
    SpriteFont font,
    string text,
    float scale,
    out bool multilineGlyphs)
  {
  ...
  private string TryReplaceGlyph(string glyph)
  {
    switch (glyph)
    {
      ...
      default:
        MappedAction key;
        if (GlyphTextRenderer.ActionMap.TryGetValue(glyph, out key))
        {
          glyph = "P_" + (object) (Keys) (this.IgnoreKeyboardRemapping ? (int) SettingsManager.Settings.UiKeyboardMapping[key] : (int) SettingsManager.Settings.KeyboardMapping[key]);
          break;
        }
        break;
    }
  ...
  private string TryReplaceGlyphSony(string glyph)
  {
    switch (glyph)
    {
      ...
      case "RT":
      case "LT":
        return GamepadState.Layout == GamepadState.GamepadLayout.PlayStation4 ? glyph + "_PS4" : glyph + "_PS3";
      default:
        return glyph;
    }
  }
  ...
  private string FindButtonGlyphs(string text)
  {
    ...
    string str1;
    string str2;
    for (; (num1 = text.IndexOf("{")) != -1 && text.IndexOf("}", num1) != -1; text = str1 + str2 + text.Substring(num2 + 1))
    {
      num2 = text.IndexOf('}', num1);
      string str3 = text.Substring(num1 + 1, num2 - num1 - 1);
      str1 = text.Substring(0, num1);
      if (GamepadState.AnyConnected)
      {
        MappedAction key1;
        if (GlyphTextRenderer.ActionMap.TryGetValue(str3, out key1))
        {
          Buttons key2;
          if (SettingsManager.Settings.ControllerMapping.TryGetValue(key1, out key2))
            str3 = GlyphTextRenderer.Buttonize[key2];
          if (GamepadState.Layout != GamepadState.GamepadLayout.Xbox360)
            str3 = this.TryReplaceGlyphSony(str3);
        }
      }
      else
        str3 = this.TryReplaceGlyph(str3);
      str2 = "";
      GlyphTextRenderer.GlyphDescription glyphDescription;
      if (this.ButtonImages.TryGetValue(str3, out glyphDescription))
      {
        this.bigGlyphDetected = glyphDescription.Metadata.IsTall;
        str2 = new string(' ', glyphDescription.Metadata.SpacesAfter);
        str1 += new string(' ', glyphDescription.Metadata.SpacesBefore);
      }
  ...
  public Texture2D GetReplacedGlyphTexture(string glyph)
  {
    ...
    {
      MappedAction key2;
      if (GlyphTextRenderer.ActionMap.TryGetValue(glyph, out key2))
      {
        Buttons key3;
        if (SettingsManager.Settings.ControllerMapping.TryGetValue(key2, out key3))
          glyph = GlyphTextRenderer.Buttonize[key3];
        if (GamepadState.Layout != GamepadState.GamepadLayout.Xbox360)
          glyph = this.TryReplaceGlyphSony(glyph);
      }
    }
    else
      glyph = this.TryReplaceGlyph(glyph);
    GlyphTextRenderer.GlyphDescription glyphDescription;
    if (this.ButtonImages.TryGetValue(glyph, out glyphDescription))
      return glyphDescription.Image;
    Logger.LogOnce(nameof (GlyphTextRenderer), LogSeverity.Warning, "Could not find glyph in button images : " + glyph);
    return this.ButtonImages[key1].Image;
  }

  public string FillInGlyphs(
    string text,
    out List<GlyphTextRenderer.FilledInGlyph> glyphLocations)
  {
    ...
    string str1;
    string str2;
    for (; (num1 = text.IndexOf("{")) != -1; text = str1 + str2 + text.Substring(num2 + 1))
    {
      num2 = text.IndexOf('}', num1);
      string str3 = text.Substring(num1 + 1, num2 - num1 - 1);
      str1 = text.Substring(0, num1);
      ...
      if (this.ButtonImages.TryGetValue(str3, out glyphDescription))
      {
        ...
        str2 = new string('^', glyphDescription.Metadata.SpacesAfter);
        str1 += new string('^', glyphDescription.Metadata.SpacesBefore);
        num3 = glyphDescription.Metadata.SpacesAfter + glyphDescription.Metadata.SpacesBefore;
      }
  ...
  public void DrawShadowedText(
    ...
    float scale)
  {
    this.DrawShadowedText(batch, font, text, position, color, scale, Color.Black, 0.333333343f, 1f);
  }
  ...
  public void DrawStringLFLeftAlign(
    SpriteBatch batch,
    SpriteFont font,
    string text,
    Color color,
    ...
    bool shadow)
  {
    ...
    float y = this.MeasureWithGlyphs(font, text, scale).Y;
    int startIndex1 = 0;
    int num2 = 0;
    ...
    int num3 = this.game.GraphicsDevice.Viewport.Width / 2;
    int startIndex2 = 0;
    Vector2 vector2 = offset + (float) num3 * Vector2.UnitX;
    while (startIndex2 < text.Length && startIndex2 != -1)
    {
      int num4 = text.IndexOf("\r\n", startIndex2);
      string text1 = text.Substring(startIndex2, num4 == -1 ? text.Length - startIndex2 : num4 - startIndex2);
      float x = this.MeasureWithGlyphs(font, text1, scale).X;
      if (shadow)
        this.DrawString(batch, font, text1, vector2 - new Vector2(x / 2f, 0.0f) + new Vector2(1f, 1f), new Color(0, 0, 0, (int) (byte) ((uint) color.A / 2U)), scale);
      this.DrawString(batch, font, text1, vector2 - new Vector2(x / 2f, 0.0f), color, scale);
      vector2.Y += y / (float) num2;
      startIndex2 = num4;
      if (startIndex2 != -1)
        startIndex2 += 2;
    }
  ...
  public Vector2 Margin
  {
    get
    {
      ...
      double x = (double) viewport.Width * 0.15000000596046448 / 2.0;
      viewport = this.game.GraphicsDevice.Viewport;
      double y = (double) viewport.Height * 0.15000000596046448 / 2.0;
  ...
  public Vector2 SafeViewportSize
  {
    ...
    {
      Viewport viewport = this.game.GraphicsDevice.Viewport;
      double x = (double) viewport.Width * 0.85000002384185791;
      viewport = this.game.GraphicsDevice.Viewport;
      double y = (double) viewport.Height * 0.85000002384185791;
      return new Vector2((float) x, (float) y).Round();
    }
  }

  private struct GlyphDescription(Texture2D image, GlyphTextRenderer.GlyphMetadata md)
  {
    public Texture2D Image = image;
    public GlyphTextRenderer.GlyphMetadata Metadata = md;
  }
