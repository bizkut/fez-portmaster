...
public static class Easing
{
  ...
  public static float EaseInOut(
    double linearStep,
    EasingType easeInType,
    float acceleration,
    EasingType easeOutType,
    float deceleration)
  {
    return linearStep >= 0.5 ? MathHelper.Lerp((float) linearStep, Easing.EaseInOut(linearStep, easeOutType), deceleration) : MathHelper.Lerp((float) linearStep, Easing.EaseInOut(linearStep, easeInType), acceleration);
  }
  ...
  private static class Sine
  {
    ...
    public static float EaseOut(double s) => (float) Math.Sin(s * 1.5707963705062866);
  ...
  {
    public static float EaseIn(double s, int power) => (float) Math.Pow(s, (double) power);
  ...
  {
    public static float EaseIn(double s) => (float) -(Math.Sqrt(1.0 - s * s) - 1.0);

    public static float EaseOut(double s) => (float) Math.Sqrt(1.0 - (s - 1.0) * s);

    public static float EaseInOut(double s)
    {
      s *= 2.0;
      return s < 1.0 ? Easing.Circular.EaseIn(s) / 2f : (float) (Math.Sqrt(1.0 - (s - 2.0) * s) + 1.0) / 2f;
    }
  }
}
