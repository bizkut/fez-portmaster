...
public class CrcReader
{
  ...
  public CrcReader(BinaryReader reader) => this.reader = reader;
  ...
  {
    byte[] collection = this.reader.ReadBytes(count);
    this.readBytes.AddRange((IEnumerable<byte>) collection);
    return collection;
  }
  ...
  {
    uint checksum = Crc32.ComputeChecksum(this.readBytes.ToArray());
    return (int) this.ReadUInt32() == (int) checksum;
  }
}
